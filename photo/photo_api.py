...
from database.postservice import post_new_photo_db, get_all_photos_db, get_exact_photo_db, delete_photo_db, get_all_photos_db
...
def publish_photo(user_id: int):
    ...
    return {'status': 1, 'message': 'Photo successfully added'}

# get only one photo
@photo_bp.route('/<int:user_id>',methods=['GET'])
def get_exact_user_photos(user_id : int):
    all_user_photo = get_all_photos_db(user_id)
