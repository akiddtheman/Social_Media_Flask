...
def change_user_post(post_id: int):
    new_post_text = request.files.get('new_post_text')

    change_post_text_db(post_id, new_post_text)

    return {'status': 1, 'message': 'Post succesfully added'}
...
def create_post(post_text: str, user_id: int):
    ...
    # Если из фронт части пришли хэштеги
    if hashtags:
        create_post_for_hashtag(new_post_id, hashtags)

    return {'status': 1, 'message': 'Post add'}
